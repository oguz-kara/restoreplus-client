...
export const POST = async (req: NextRequest, res: NextResponse) => {
  try {
    ...
    const { data } = await serverFetcher('/product/documentations', {
...
export const GET = async (req: NextRequest) => {
  try {
    ...
    if (id) {
      ...
      const response = await serverFetcher(`/product/documentations/${id}`)
      const { data: responseData } = response
      return NextResponse.json(responseData, { status: 201 })
    }

    const response = await serverFetcher('/product/documentations/all')
    const { data: responseData } = response
